...
### 运行效果
运行 `crawler.py` 处理全部 323 个菜品（跳过先前已成功的菜品，断点续传），最终结果：
...
| | Web 版（v0.1.0） | API 版（v0.2.0） |
|---|---|---|
| 成功 | 23 | **219** |
| 成功率 | 7.1% | **67.8%** |
| 核心问题 | 403 反爬 | 基本解决 |

API 版从最初的 7.1% 提升到 67.8%，彻底消除了 403 反爬问题。剩余的 104 个跳过菜品主要是非标准菜名（如「水油焖蔬菜」「雷椒皮蛋」等）在百度百科中无对应词条。
